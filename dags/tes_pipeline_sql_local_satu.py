...
default_args = {
    'owner': 'mysql_user',  # Change this to your MySQL username
    'depends_on_past': False,
    'email_on_failure': False,
    'email_on_retry': False,
    'start_date': datetime(2023, 10, 30, tzinfo=local_tz),
    'retries': 5,
    'retry_delay': timedelta(minutes=1)  # Wait for 1 minute before retrying
}
...
def get_weather_data_by_day_and_time(day, time, cur):
    # SQL query to retrieve data based on day and time
    sql = "SELECT * FROM cuaca_jam WHERE tanggal = %s AND jam = %s"
    val = (day, time)
    cur.execute(sql, val)
    result = cur.fetchone()
    return result
...
def extract(**kwargs):
    ...
    for jam_ex in jam_ex_list:
        ...
        if existing_data is None:
            ...
            raise Exception("Data with the same day and time already exists in the database.")
...
def load(**kwargs):
    ...
    for data in data_list:
        ...
        if existing_data is None:
            sql = """INSERT INTO cuaca_jam (tanggal, jam, temperature, feelslike, humidity, dew_point, wind_gust, wind_speed, wind_direction, pressure, visibility, cloud_cover, solarradiation, solarenergy, uv_index, conditions, icon, source, tanggal_pengambilan, jam_pengambilan)
...
with DAG('weather_etl_mysql_docker_satu',
